...
(defn length [segment]
  (cond
    (horizontal? segment) (apply - (reverse (sort (hproj segment))))
    (vertical? segment) (apply - (reverse (sort (vproj segment))))))

(defn distance-along [segment point]
  (cond
    (horizontal? segment)
    (abs (- (first point) (first (first segment))))
    (vertical? segment)
    (abs (- (second point) (second (first segment))))))

(defn intersections [blue red]
  (for [b blue r red
        :let [i (intersection b r)]
        :when (some? i)] i))

(defn distance-to [wire point]
  (loop [so-far 0
         remaining-wire wire]
    (let [segment (first remaining-wire)]
      (if (intersection segment [point point])
        (+ so-far (distance-along segment point))
        (recur (+ so-far (length segment)) (rest remaining-wire))))))

(defn distances [blue red]
  (->> (intersections blue red)
       (filter #(not= % [0 0]))
       (map #(+ (distance-to blue %) (distance-to red %)))))

(distances (first wires) (second wires))
