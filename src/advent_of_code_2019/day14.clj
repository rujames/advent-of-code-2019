...
(defn parse-reaction [line]
  (letfn [(parse-quantity [x] (#(hash-map :quantity (read-string (first %)) :symbol (second %)) (str/split x #" ")))]
    (let [[inputs output] (mapv str/trim (str/split line #"\=\>"))
          inputs (mapv parse-quantity (str/split inputs #"\, "))
          output (parse-quantity output)]
      [(:symbol output)
       {:output output
        :inputs inputs}])))

(defn take-from-stockpile [stockpile input]
  (let [{:keys [symbol quantity]} input
        in-stockpile (get stockpile symbol 0)]
    (if (<= quantity in-stockpile)
      [(assoc stockpile symbol (- in-stockpile quantity))]
      [(dissoc stockpile symbol) (assoc input :quantity (- quantity in-stockpile))])))
...
(defn produce [reactions stockpile chemical]
  (if-let [reaction (reactions (:symbol chemical))]
    (let [multiplier (bigint (Math/ceil (/ (:quantity chemical) (:quantity (:output reaction)))))
          ...
          [stockpile cost]
          (reduce
           (fn [[stockpile cost] input]
             (let [[stockpile cost-increase]
                   (request reactions stockpile input)]
               [stockpile (+ cost cost-increase)]))
           [stockpile 0]
           inputs)]
      [(assoc stockpile (:symbol chemical) excess) cost])
    [stockpile (:quantity chemical)]))

(defn request [reactions stockpile chemical]
  (let [[stockpile remainder] (take-from-stockpile stockpile chemical)]
    (if remainder
      (produce reactions stockpile remainder)
      [stockpile 0])))

(def reactions
  (->> (str/split (slurp "src/advent_of_code_2019/day14.input") #"\r\n")
       (map parse-reaction)
       (into {})))
